...
###########################REVIEW############################################
#crimes_df <-  subset(read.csv("la_crime_data/pre_processed/reformed_for_lasso_sum_all.csv"), select=c('census', 'volume'))
#crimes_df <- subset(read.csv("la_crime_data/pre_processed/reformed_for_lasso_sum_violence.csv"), select=c('census', 'volume'))
#crimes_df <- subset(read.csv("la_crime_data/pre_processed/reformed_for_lasso_sum_property.csv"), select=c('census', 'volume'))
#crimes_df <- subset(read.csv("la_crime_data/pre_processed/census_conditional_violence_shannon.csv"), select=c('census', 'crimes_shannon_index'))

#destination = "F:/Dropbox/Dr Mohler/Crime discovery/la_crime_data/review/CrimeVolReg/all"
#destination = "F:/Dropbox/Dr Mohler/Crime discovery/la_crime_data/review/CrimeVolReg/violence"
#destination = "F:/Dropbox/Dr Mohler/Crime discovery/la_crime_data/review/CrimeVolReg/property"
########################################################################
...
data_census = data[, (colnames(data) %in% c("census"))]

write.csv(data_census, file ="la_crime_data/review/census.csv")

dataset <- data#count(data, vars = colnames(data)) ##### ALL DATA####
...
subset_dataset <- dataset[, !(colnames(dataset) %in% c("census", "","lat", "lon", "volume", "freq", "B01003_HD01_VD01", "C17002_HD01_VD08", "B02001_HD01_VD08"))]#subset(dataset, select=c('labels_shannon_index', 'population', 'income', 'household_shannon_index', 'language_shannon_index', 'race_shannon_index'))
...
scaled_data$volume<-dataset$volume

scaled_data <- data.frame(center_scale(scaled_data))


#########################################
#model_lm=lm(volume~labels_shannon_index+
#               household_shannon_index+language_shannon_index+population+ALAND,
#             data=scaled_data)
#lm_pred=predict(model_lm,scaled_data)

# co <- dataset[, c('lon', 'lat')]
# GWRbandwidth <- gwr.sel(volume~., data=scaled_data, coords=as.matrix(co),adapt=T) 
#########################################
...
plot_density(scaled_data, scaled_data$volume, "density", "crimes volume", "Density of crimes volume in data")
...
plot_histogram(scaled_data, scaled_data$volume, "crimes volume", "Histogram of crimes volume")
...
flm<-as.formula(paste("volume ~ " , columns_str, sep=""))
#Linear regression
...
plot_density(scaled_data, linear_pred, "density", "predicted crimes volume with linear regression", "Density of predicted crimes volume with linear regression")
...
#Plot histogram of crime diversity
plot_histogram(scaled_data, linear_pred, "predicted crimes volume with linear regression", "Histogram of predicted crimes volume with linear regression")

#Plot the predicted value against the actual value

jpeg(file = paste(destination, "Linear predicted crimes volume vs actual.jpeg", sep="/"))
plot(linear_pred,scaled_data$volume, xlab="predicted crimes volume using linear regression",ylab="actual crimes volume")
dev.off()
...
par(mfrow=c(1,1))
hist(plot_df$volume, # histogram
     col="gray", # column color
     border="black",
     prob = TRUE, # show densities instead of frequencies
     xlab = "volume",
     cex.axis=0.5, font.main=1
     ,main = "Histogram of actual crime volume along with density of predicted crime volume using Linear Regression"
)
...
hist(plot_df$volume, # histogram
     ...
     xlab = "volume",
     cex.axis=0.5, font.main=1
     ,main = "Histogram of actual crime volume along with density of predicted crime volume using GWR"
)
...
#Plot the predicted value against the actual value
plot(gwr_pred, scaled_data$volume, xlab="GWR predicted crimes volume", ylab="actual crimes volume")
abline(a=0,b=1)
dev.off()

jpeg(file = paste(destination, "GWR residuals against gwr value.jpeg", sep="/"))
#Plot the residuals against predicted value
plot(results$gwr.e, gwr_pred, xlab="GWR residuals", ylab="GWR predicted crimes volume")
abline(a=0,b=1)
dev.off()

#jpeg(file = paste(destination, "density of predicted gwr.jpeg", sep="/"))
#Plot density of predicted crime diversity
plot_density(scaled_data, gwr_pred, "density", "predicted crimes volume with GWR", "Density of predicted crimes volume with GWR")
#dev.off()
...
plot_histogram(scaled_data, gwr_pred, "predicted crimes volume with GWR", "Histogram of predicted crimes volume with GWR")
#dev.off()
...
# now create the map
ggplot(scaled_data, aes(x=lon,y=lat))+geom_point(aes(colour=scaled_data$volume))+scale_colour_gradient2(low = "red", mid = "white", high = "blue", midpoint = 0, space = "rgb", na.value = "grey50", guide = "colourbar", guide_legend(title="range"))
