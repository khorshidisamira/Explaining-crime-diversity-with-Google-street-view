...
#??????????????????????
commoncrime_label = pd.Series(list(set(crime_census) & set(label_census)))
common = pd.Series(list(set(household_census) & set(language_census) & set(race_census) & set(income_census) & set(population_census) & set(crime_census)))
#?????????????????????????
...
"""
with open('la_crime_data/dataset.dat', 'wb') as outfile:
